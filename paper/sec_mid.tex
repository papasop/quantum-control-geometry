...
preconditioner. The regularity of each frozen point preconditioner is separately
certified by an independent $256$-bit outward-rounded Rump--Neumann argument: for all
twelve paths $\lVert I-R_{k}C_{k}\rVert_{\infty}<1$, so each $C_{k}$ is nonsingular
(certificate \nolinkurl{results/audit_closure/p0_preconditioner_certificate.json}). The
Jacobian over each full box is then enclosed with 192-bit Arb
