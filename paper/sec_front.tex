...
Two separate mechanism certificates, not used in the direct theorem, partially
recover this ordering without direct finite-error propagation. An order-thirty zero-error
Taylor jet, equipped with Cauchy-alias and Dyson-tail enclosures, certifies 52/66
comparisons with no certified reversal; its quartic term alone certifies 34/66. On a
...
$\rho_{\mathrm{Spearman}}\ge0.95$ & prospective evidence (separate cohort)
\end{tabular}
\end{center}
The prospective and formal results use disjoint frozen cohorts; the former is not a
premise of the latter.
...
For a declared tolerance $\eta$, the numerical matched fibre $\Ff_{\eta}$ consists of
controls $\gamma$ satisfying
\begin{equation}\label{eq:fibre}
\Iref_{\gamma}(0)\le\eta,\qquad
...
\begin{scope}[xshift=0cm]
  ...
  \draw[gray,dashed,thick] (0.95,0.95) -- (3.25,2.9)
        node[gray,anchor=west,font=\small] {numerical tangent};
  ...
  \node[red,align=center,anchor=west] (bl) at (2.9,0.95)
        {schematic projection of\\ transverse Krawczyk box $X_{k}$};
  \draw[red,->] (bl.west) -- (2.55,1.55);
  \node[blue,align=center] at (1.4,-0.7) {local numerical\\ continuation};
...
\begin{scope}[xshift=8.8cm]
  ...
  \draw[dotted,thick] (-1,0) -- (-1,2.35);
  \node[anchor=south,font=\small] at (1,2.36) {$t{=}{+}1$};
  \node[anchor=south,font=\small] at (-1,2.36) {$t{=}{-}1$};
