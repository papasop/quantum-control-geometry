...
robustness. We study this
residual implementation dependence in a serialized two-atom Rydberg model, a minimal
interacting setting in which amplitude, detuning, and interaction-calibration responses
coexist, with twenty-four control phases and three global error coordinates.

A fourth-order zero-point response coefficient is first frozen as a prospective ranking
rule. In the predeclared held-out twenty-path cohort, it achieves Spearman correlation
$0.996992$ with mean finite-error performance and identifies the best path. Additional
seeded cohorts showed appreciable variation in the prospective correlation, so the quartic
rule is treated as empirical evidence rather than a universal ranking theorem. The
coefficient is then represented as the contraction of a symmetric fourth-order response
tensor in the three-dimensional error space with the fourth noise moment; direct
recomputation under a nonorthogonal coordinate change verifies the associated covariance
and contraction invariance.

For a separate twelve-path cohort, whose target order was frozen by its own pre-outcome
order-thirty point calculation, independently of the twenty-path quartic study, strict
192-bit Krawczyk inclusions certify locally unique roots whose projective output states and
first projective responses exactly match the reference. Direct outward-rounded Arb
propagation of the certified root boxes through the declared six-axis error functional then
proves the complete finite-error ordering for all 66 unordered path pairs, agreeing with an
order frozen before the direct propagation. A separate order-thirty local-jet calculation
certifies 52/66 comparisons with no reversed certified pair, while the quartic term alone
certifies 34/66.

The main result is therefore a computer-assisted exact-root finite-error ordering theorem
for the declared finite-dimensional model. It does not constitute hardware,
model-discrepancy, global-fibre, or many-body evidence.
\end{abstract}
...
designated low-order sensitivities of a control. Here we instead condition on a strong
...
\end{tabular}
\end{center}
The 66/66 direct exact-root certificate is the main theorem. The 52/66 order-thirty result
explains how much of that ordering is resolved by a finite local jet. The 34/66 quartic
certificate marks the limit of the lowest-order geometric description. The Spearman result
is prospective evidence obtained on a separate held-out twenty-path cohort; the frozen
order used in the exact-root theorem is supplied by the pre-outcome order-thirty
certificate for the twelve-path cohort, and no pathwise identity between the two cohorts is
claimed.
