...
The result is a formal ordering certificate for a serialized-decimal,
finite-dimensional Hamiltonian model.  It is not evidence from PASQAL
hardware, and exact existence of the numerically matched implementation
fibre remains to be certified by an interval-Newton or Krawczyk argument.
\end{abstract}
...
analytic tail bound certifies the complete finite-error ordering.  This
distinguishes three statements that are often conflated:
\[
\text{low-order correlation}
\;\neq\;
\text{partial pairwise certification}
\;\neq\;
\text{complete bounded reconstruction}.
\]
...
\begin{remark}[Unfinished exact-fibre step]
The numerical rank and residual establish a highly accurate computational
fibre, not the exact existence of an eight-dimensional solution manifold.
A complete exact-fibre theorem requires a square transverse formulation and
an interval-Newton or Krawczyk inclusion proof.  This limitation is kept
separate from the formal ordering certificate below.
\end{remark}
...
discrepancy, calibration drift, sampling noise, or PASQAL hardware.  It also
does not prove that the numerical paths lie on an exact first-order matched
fibre; it proves the finite-error ordering of those serialized controls.
\end{remark}
...
\begin{enumerate}[leftmargin=2.2em]
\item \textbf{Exact-fibre existence.}
The matching constraints are satisfied to numerical tolerance.  A formal
interval-Newton/Krawczyk proof has not yet established a nearby exact matched
...
The next mathematical step is to decompose the 24-dimensional phase space
into sixteen transverse variables and eight fibre variables, formulate an
independent analytic constraint map, and prove a Krawczyk inclusion
\[
K(X)\subset\operatorname{int}(X)
\]
for a box $X$ around each numerical path.  Propagating the resulting phase
balls through the formal jet calculation would close the chain
\[
\text{exact matched control}
\;+\;
\text{bounded local jet}
\;\Longrightarrow\;
...
with a formal remainder enclosure.  Establishing exact matched-fibre
existence and extending the certificate to open-system, many-atom, and
hardware data are the remaining steps.
