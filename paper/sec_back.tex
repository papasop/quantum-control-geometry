...
\caption{Direct visual evidence for the prospective ranking result and the exact-root
certification theorem. (a) Prospective
prediction: $-\Gg_{4}$ predicted versus actual ranks on the held-out twenty-path cohort.
The plotted annotation is the original Colab historical sample; cross-architecture reruns
of the same frozen float64 prospective pipeline preserve the predeclared
$\rho_{\mathrm{Spearman}}\ge0.95$ gate while shifting the exact correlation and named top
path. This panel reports prospective empirical evidence; it is not a certificate. (b)
...
Figure~\ref{fig:evidence} gives the two corresponding data-level statements. One
distinction must be made explicit: the frozen-point order-30 audit certifies 66/66
comparisons before propagation over the Krawczyk boxes and is not an exact-root
...
On a separate twenty-path cohort, the frozen quartic contraction predicts the held-out
ranking prospectively above the predeclared $\rho_{\mathrm{Spearman}}\ge0.95$ gate; this
threshold-level evidence is an application of the same structure, not a premise of the
theorem.
