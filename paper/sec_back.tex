...
Table~\ref{tab:audits} collects the audits in the fixed hierarchy of the paper: the 66/66
direct exact-root ordering theorem (main result), supported by the 12/12 Krawczyk local
uniqueness certificates; the 52/66 order-thirty mechanism certificate; the 34/66
quartic-only boundary; and the prospective $\rho=0.996992$ evidence on the separate
cohort. A frozen-point diagnostic sharpens the picture: for the twelve serialized
numerical phase points, before propagation over the Krawczyk root boxes, the order-thirty
jet and its analytic tail enclosure certify all 66/66 pairwise comparisons. This pointwise
diagnostic is witnessed by the proof objects in \texttt{results/l4\_order30/}; it is not an
...
comparison: it certifies 34/66 pairs alone, and a zero-point jet through order thirty
